...
> Loops v1 已可用且经验证，以下为健壮性、可维护性、可移植性、可观测性的改进空间。round 12 复审未发现新的非 SSO/file 待优化实现项。

## 本轮（round 12 / 2026-06-20）进度

round 12 复审未发现新的非 SSO/file 待优化实现项。关键变化：回归复核确认 `docs/0619/sso` 迁移（已至第十二轮）已修复此前残留的 auth/SSO architecture 命中，`OPT-10` / `OPT-11` 边界注释中「剩余命中在 auth/SSO 范围」「quality:gate 仍被 auth/SSO 短路」已失效——`pnpm check:architecture` 与 `pnpm quality:gate` 现全绿。`OPT-3` / `OPT-5` 维持 accepted：`generated/db` 仍处 SSO/file 并行迁移脏工作树（34 个文件未提交），本轮不裁剪 per-model 生成产物以免覆盖另一进程状态。
...
| OPT-3 裁剪冗余 per-model 生成 service | 🟡 accepted   | round 10 复核无业务引用；保留生成产物，继续约束实际访问走 `LoopsDbService`                         |
...
round 12 复审结论：`OPT-3` / `OPT-5` 继续维持 accepted，非阻断；`OPT-1/2/4/6/7/8/9/10/11` 均已 done/documented。round 12 确认 auth/SSO architecture 命中已被 `docs/0619/sso` 迁移修复，`check:architecture` / `quality:gate` 全绿。当前没有需要在本目录继续实施的非 SSO/file 优化项。
...
## OPT-3 · 裁剪未被使用的 per-model 生成 DB Service【🟡 accepted】

- 现状：`generated/db/modules/loop-issue*`、`loop-state*`、`loop-issue-intake*` 是生成器产物，实际 Loops DB 访问集中在手写 `generated/db/modules/loops/loops-db.service.ts`。
- 决策：暂不裁剪生成产物，避免每次 `pnpm db:generate` 又重建并引入额外生成器分支；通过文档约束继续要求 Loops 业务访问走 `LoopsDbService`。
- round 10 复核：`rg "LoopIssueService|LoopStateService|LoopIssueIntakeService" apps/api/src apps/api/libs packages scripts -g'*.ts' -g'*.js'` 无命中，确认业务代码没有直接消费这些 per-model 生成 service。
- 边界：当前 `apps/api/generated/db/**` 正处于 SSO/file 并行迁移与生成产物脏工作树中；本目录不直接删除生成文件，避免覆盖另一进程的 DB 生成状态。

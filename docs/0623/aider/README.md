...
| 优先级 | 建议                     | 验收                                                                              |
| ------ | ------------------------ | --------------------------------------------------------------------------------- |
| P1     | Workspace repo map       | Spec/decompose 可复用代码地图                                                     |
| P1     | Git evidence panel       | 部分实施：Loop Board v1 展示派生 branch 与 PR 状态；commits/dirty state 后续 Epic |
| P1     | Test command policy      | 每个 repo 配置默认 unit/type/lint/e2e                                             |
| P2     | Voice/image issue intake | 面向非工程用户补充输入方式                                                        |

## 实施标注

2026-06-23 已在 Loop Board v1 中加入 branch / PR 状态的第一层展示。真实 commit range、dirty state、repo map 与 test command policy 仍需要 git/workspace profile contract，归入后续 Epic。

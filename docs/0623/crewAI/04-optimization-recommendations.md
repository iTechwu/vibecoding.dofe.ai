...
实施状态：权限底座、artifact provider 与队列 processor v1 已推进。SSO Asset Permissions v1 已覆盖 `remote-runner`、`mcp-server`、`ci-check` 三类高风险资产；现有 `runtime-backend` health-check / policy patch、R10 MCP Server Registry / CI Checks Registry、R12 Remote Runner pool/lease 控制面，以及 R21 Remote Runner job worker/artifact manifest 写操作均已复用 SSO asset permission 硬门禁。R21 已支持 `POST /loops/remote-runners/:id/jobs` 并真实写入 job 索引与 manifest artifact；R34a 已新增 BullMQ `loops-remote-runner` processor；R36 已新增 Remote Runner artifact external upload endpoint。后续需接真实 CLI adapter 执行、取消/续跑、sandbox worker 深度日志与 org quota。

从本地/Docker 走向团队产品必须补：

- queue worker（R12 已完成控制面 lease，R21 已完成 job/artifact manifest provider v1，R34a 已有 BullMQ processor；真实执行编排后续）；
- job lease（R12 已完成 acquire/release lease contract，R21 已完成 job request/response contract）；
- concurrency；
- cancellation；
- resumability；
- artifact manifest provider（R21 已完成；R36 已有码外部 object storage upload，生产配置和权限治理后续）；
- sandbox logs；

...
- 队列化后台 worker 已用于默认 `advance`：Controller 在 SSO scope 授权后入队，单并发 BullMQ worker 执行既有状态机，失败按指数退避重试三次；完成或最终失败后移除 job，使同一 Issue 可以继续进入下一次产品级推进。生命周期投影持久化在 Redis，`GET /issues/:issueId/advance-status` 提供轮询读取，`GET /issues/:issueId/advance-events` 以 SSE 推送状态变化且在建连前验证 tenant ownership。Detail 页消费该 SSE 流，刷新受影响的 React Query 缓存，并以 4 秒轮询作为断线回退。默认 `/loops` 页面现为对话式工作台：一条新需求创建一个 Issue，并在同一消息流中显示状态；运营面板保留在 `/loops?view=operations`。CLI 和兼容 service 调用仍保留同步语义。

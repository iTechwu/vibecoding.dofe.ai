...
| Step 6 · Eval / Bench                       | 🚧   | Eval aggregation worker、eval suite/run builder、eval trend baseline builder、request-time aggregation builder、loop bench metric helper + eval/bench trend worker IO 编排 + eval aggregation worker 编排 → `loops-eval`；evidence 收集与 DB/Redis 适配仍由 facade port 实现 | Cycle 38–41, nextstep 63–66 |
...
| Step 8 · Trigger / Remote Runner            | 🚧   | schedule trigger CRUD + `fireScheduleTrigger` 编排 → `loops-triggers`（issue creation port `LOOPS_ISSUE_CREATION_PORT` 当前由 facade 临时实现）；remote runner list/lease/job → `loops-remote-runners`；remote shard execution pipeline 仍在 facade                          | Cycle 13–15, nextstep 58–60 |

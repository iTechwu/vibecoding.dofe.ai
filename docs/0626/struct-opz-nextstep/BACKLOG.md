...
| 编号 | 领域                          | 当前状态                                                                                                                                                                                                                             | 后续动作                                                                                                                             | 风险             |
| ---- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------- |
| N1   | Step 3 Engine 主流程          | 纯推导 + 谓词 + `generateSpec`/`decompose` + `advance` 递归调度 + `runLoopUnlocked` shard 调度 + `runRunnableShard` 重执行 + `reloop`/`applyResume`/`finalize`/`reviewGlobal`（经各 port 注入重依赖 builder）已下沉到 `loops-engine` | 继续迁 `runLoop` workLock 包装；finalize/reviewGlobal/retry builder 仍由 facade port 提供                                            | 高（多批进行中） |
...
| N4   | Step 8 Remote Execution       | list/lease/job + artifact IO（`uploadRemoteRunnerArtifacts` + `LoopsRemoteArtifactStoragePort`）+ shard execution job lifecycle（implement/test/review 编排）已下沉到 `loops-remote-runners`；processor 经 token 解耦 facade         | 补 processor/Nest provider focused spec；评估 CLI/Docker runtime adapter 是否需要独立 API adapter class，Step N7 删除 facade wrapper | 中               |
...
1. N1 Engine workLock 包装：N1 最后一段，完成后可解锁 Step N7 facade 收敛。
...
- Engine 主流程剩余 workLock 包装仍是高风险项：依赖 locks + state mutation，必须单独循环。

  ...
  async run(input: {
    ...
    try {
      ...
      return {
        ...
        visualDiffs: input.request.viewports.map((viewport) => ({
          baselinePath: `${input.baselineRef}/${viewport.name}`,
          actualPath: `${input.screenshotRef}/${viewport.name}`,
          diffPath:
            result.visualStatus === 'changed' ? `${input.diffRef}/${viewport.name}` : undefined,
          status: result.visualStatus,
          label: `page-load · ${viewport.name} ${viewport.width}x${viewport.height}`,
          viewport,
        })),

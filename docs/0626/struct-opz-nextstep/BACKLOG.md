...
| 编号 | 领域                          | 当前状态                                                                                                                                                                                                                                                                                             | 后续动作                                                                                                           | 风险 |
| ---- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ---- |
| N1   | Step 3 Engine 主流程          | 纯推导 + 谓词 + `generateSpec`/`decompose` + `advance` 递归调度 + `runLoop` workLock 包装 + `runLoopUnlocked` shard 调度 + `runRunnableShard` 重执行 + `reloop`/`applyResume`/`finalize`/`reviewGlobal`（经各 port 注入重依赖 builder）已下沉到 `loops-engine`                                       | N7 前继续收敛 facade builder ports；finalize/reviewGlobal/retry builder 仍由 facade port 提供                      | 中高 |
...
| N4   | Step 8 Remote Execution       | list/lease/job + artifact IO（`uploadRemoteRunnerArtifacts` + `LoopsRemoteArtifactStoragePort`）+ shard execution job lifecycle（implement/test/review 编排）已下沉到 `loops-remote-runners`；processor/skipped CLI e2e 经 execution port 解耦 facade；`executeRemoteShardJob` facade wrapper 已删除 | 后续仅保留 remote runner 非执行面 wrapper 收敛                                                                     | 低   |
...
| N7   | Step 10 Facade 收敛           | 已启动：remote execution port 不再由 `LoopsService implements` 提供；remote runtime/state/detail/log adapters 不再经 facade 桥接；`executeRemoteShardJob` direct-call wrapper 已删除                                                                                                                 | 继续删除其他已迁 wrapper，收敛 API module providers                                                                | 中高 |
...
1. N7 Step 10 facade 收敛继续推进：以 N1 runLoop port 与 N4 runtime adapter 为基础，删除已迁 wrapper。
...
5. N3 Trigger processor focused 子集：issue creation port 已下沉，可补 schedule tick → fire 端到端 focused spec。
6. N4 remote runner 非执行面 wrapper 清理：继续评估 list/lease/job/artifact wrappers 的 controller 兼容迁移。

## 当前风险提醒

- Engine 主流程 workLock 包装已迁入 `LoopsEngineService.runLoop`；剩余风险是 facade builder ports 仍承载重依赖，删除 wrapper 前必须逐项对齐 tests。
...
- Trigger fire 编排已下沉到 `loops-triggers`，processor 不再注入 legacy facade 类；issue creation port 已绑定 `LoopsIssuesService`，剩余风险是 processor schedule tick → fire focused 子集。

...
| Step 3 · Engine 状态机                      | 🚧   | 纯推导 + 谓词 + generateSpec/decompose + advance 递归调度 + runLoopUnlocked shard 调度（recover/block/converge，`LoopsEngineShardRunnerPort`）→ `loops-engine`；finalize/reloop/reviewGlobal/resumeAndRead + runLoop workLock 包装 + runRunnableShard 重执行仍在 facade（N1 分子批迁移）                                                          | Loop 7, nextstep 82–89                 |

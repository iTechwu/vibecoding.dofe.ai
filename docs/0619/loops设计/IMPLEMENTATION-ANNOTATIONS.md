...
| DB schema + 迁移           | done                | `apps/api/prisma/schema.prisma`, `prisma/migrations/20260619120000_add_loops_persistence/migration.sql` | `LoopIssue`/`LoopIssueIntake`/`LoopState` 三表 + 索引 + 级联，迁移与 schema 对齐。                            |
| DB Service / DB 读写       | done                | `apps/api/generated/db/modules/loops/loops-db.service.ts` 等                                            | `LoopsDbService`（事务化三表写入 + list/upsert/update/detail）；Loops Service/Controller 无直接 Prisma 访问。 |
| Contract / API 表面        | done                | `packages/contracts/src/schemas/loops.schema.ts`, `.../api/loops.contract.ts`, `loops.controller.ts`    | Zod-first contract；`POST/GET /loops/issues`；submitter 服务端默认 dev；列表符合分页规范。                    |
| `.loops` 文档真相源        | done                | `loops-file-store.service.ts`, `.loops/config.yaml`                                                     | Issue/Intake/State/Spec/Shard/Test/Review/Annotation/log/Notification 文件存储与文件 doctor 可用。            |
| DB + `.loops` 双写         | done                | `loops-persistence.service.ts`, `loops.service.ts`                                                      | `createIssue` 双写、列表 DB 优先、详情合并、`syncState`/`syncClosed` 同步；DB 写运行期待迁移后验证。          |
| Web 无登录提交             | done                | `apps/web/app/loops/new`, `apps/web/lib/api/loops.ts`                                                   | v1 最简表单（无 submitter 字段）、提交跳转、API-backed 队列/详情；web type-check 通过。                       |
| Loop 生命周期 DB 状态同步  | done                | `loops.service.ts`（`syncAndRead`）、`loops-persistence.service.ts`                                     | 各阶段经 `syncAndRead` 同步 DB phase/shards/cost/finalized；finalize 同步 CLOSED/finalized。                  |
| Loop 完成 CLOSED/finalized | done                | `LoopsService.finalize`, `LoopsFileStoreService.writeFinalize`, `syncAndRead`                           | 文件 + DB 两侧均 CLOSED/finalized（DB 同步待迁移后运行期验证）。                                              |
| Doctor 一致性              | done                | `LoopsPersistenceService.doctor`, `LoopsDoctorResponseSchema`                                           | 区分 file/db/consistency 三类问题；CLI 为纯文件模式，DB 一致性经 API server 验证。                            |
| Smoke / 质量门禁           | done                | `loops.service.spec.ts`, `jest.config.ts`, `scripts/loops-cli.ts`                                       | 文件侧全链路 jest 冒烟通过 + CLI doctor/status 可运行；DB smoke 待迁移后人工验证。                            |
| SSO / 飞书 / 真实 PR       | not-started（后置） | `todo/TASK-09...`, 设计文档 v1 裁剪说明                                                                 | 未误纳入 v1；后续阶段继续推进。                                                                               |
...
| TASK-01 | done | Prisma model + 迁移 + `LoopsDbService`（事务化三表写入与查询/详情）均落地；DB 运行期写入待迁移后验证。                                         |
| TASK-02 | done | Contract/API surface 完整；服务端默认 submitter 生效；DB 优先列表符合分页规范；web/contracts type-check 通过。                                 |
| TASK-03 | done | `LoopsPersistenceService` 双写、DB 优先列表、合并详情、失败/幂等策略落地；DB 运行期写入待迁移后验证。                                          |
| TASK-04 | done | v1 无登录最简表单（已移除 submitter 字段）、提交跳转、API-backed 队列/详情完成。                                                               |
| TASK-05 | done | 生命周期各阶段经 `syncAndRead`/`syncState` 同步 DB `LoopState`/`LoopIssue.status`，finalize 同步 CLOSED/finalized；DB 运行期同步待迁移后验证。 |
| TASK-06 | done | 文件 + DB + 一致性三类 doctor 落地；DB 一致性运行期验证待迁移后进行。                                                                          |
| TASK-07 | done | 新增文件侧全链路 jest 冒烟（通过）并修复 jest 配置与 CLI；DB 侧 smoke 待迁移后人工验证，既有非 Loops 阻断已列明。                              |
| TASK-08 | done | round 2 重新归档为当前权威结论；TASK-01..08 状态据实回填。                                                                                     |
| TASK-09 | done | 后置项清单清楚，未把 SSO/飞书/真实 PR 纳入 v1。                                                                                                |

### 本轮（round 2）关键修复与验证

- 删除 `loops.service.ts` 中重复的 `syncAndRead`（原 `TS2393: Duplicate function implementation`，为 Loops 唯一 type-check 阻断）。
- `LoopsService.persistence` 改为 `@Optional() @Inject(LOOPS_PERSISTENCE)` + type-only import + 新增 `loops-persistence.token.ts`：API server 经 DI 注入走 DB 双写，独立 `ts-node` CLI（`scripts/loops-cli.ts`）不再因静态导入 `LoopsPersistenceService` 拖入 `@app/db` 而报 `Cannot find module '@app/db'`；`list()`/`getIssue()` 增加 `.loops` 文件回退，CLI 以纯文件模式运行。
...
## 最终归档（TASK-08 / round 1 / 2026-06-19）— ⚠️ 已被 round 2 取代，仅保留作历史

> 以下为持久化层落地前的早期归档，结论偏保守（DB Service/dual-write/生命周期 DB 同步当时确为 not-started）。round 1 落地完成后这些项均已升级为 done，详见上方 round 2。

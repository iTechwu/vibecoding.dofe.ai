...
- 真实 SSO 浏览器 E2E：round 2 时 blocked，已于 round 14 在本地真实联调环境解除。
...
- 真实 SSO 浏览器 E2E：round 3 时 blocked，已于 round 14 在本地真实联调环境解除。
...
- 真实 SSO 浏览器 E2E：round 12 时 blocked，已于 round 14 在本地真实联调环境解除。
...
- 真实 SSO 浏览器 E2E：已于 round 14 在本地真实联调环境解除；生产/测试环境仍需按各自凭据重新验收。
...
## round 14 回归记录（2026-06-20）

范围：

- 用户允许启动服务后，重新执行真实 SSO 浏览器 E2E。
- 复核用户点名的五类真实集成缺口：SSO、飞书入口/审批、真实 PR/diff、独立 worker 池、真实 CLI/token 计量。
- 将 B3 从 blocked 更新为本地真实联调 done；其余四类仍按实际外部依赖维持 blocked/partial。

实施文件：

- `docs/0619/sso/09-implementation-status.md`
- `docs/0620/README.md`
- `docs/0620/01-execution-plan.md`
- `docs/0620/03-deferred-items.md`
- `docs/0620/04-regression-checklist.md`
...
- 服务启动确认：`sso.dofe.ai` API `:3100`、SSO Web `:3000`、vibecoding API `:13100`、vibecoding Web `:3003` 均处于 LISTEN。
- `pnpm --filter @repo/web test:e2e:sso`：失败，原因是未提供 `E2E_SSO_EMAIL` / `E2E_SSO_MOBILE`；该脚本会强制启用真实 E2E，不能无账号执行。
...
- `pnpm type-check`、`pnpm --filter @repo/api exec jest --runInBand`、`pnpm --filter @repo/api exec jest src/modules/loops --runInBand`、`pnpm --filter @repo/contracts test`、`pnpm --filter @repo/web test`、`pnpm --filter @repo/validators test`、`pnpm quality:gate`、`pnpm regression:docs0620`：通过。

仍未完成：

- 飞书入口/审批：blocked，需真实 payload、签名、用户映射和审批状态机；当前仅有 Feishu webhook sender 前置。
- 真实 PR 环境验收与真实 diff 自动回收：blocked，需真实 provider token/仓库与 changedFiles 来源；当前仅有 GitHub/GitLab/Gitea provider client 和 push 后开 PR 前置。
- 独立 worker 池：blocked，需跨进程队列、锁、资源隔离和部署拓扑；当前仅有 API 进程内 `LoopsWorkLockService`。
- 真实 CLI 生产稳定性与真实 token 计量：blocked，需生产 CLI 策略和计量来源；当前 CLI adapter 有重试/超时/降级，但 token 仍来自 CLI JSON 或 stdout 长度估算。

Loops v1 CLOSED 门槛：不受影响；本轮只解除本地真实 SSO E2E 阻塞，不扩大其他真实外部验收口径。

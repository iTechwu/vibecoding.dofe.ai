...
> **实施后修正**：vibecoding 与 scaffold 的用户、认证、会话、文件均以 `sso.dofe.ai` 为唯一真源；本地不保留复杂 auth/file 源模型。准确落地状态见 [09-implementation-status.md](./09-implementation-status.md)。
...
- 在远程校验前，`AuthGuard` 会读取本地 `TOKEN_BLACKLIST_PREFIX + jti`。这是 logout 后的防御性拒绝层；Redis/解码异常不改变主路径，仍回落到 SSO 远程校验。
...
- 刷新：api 端点 `POST /auth/oidc/token` 只读 `dofe_rf` cookie 里的 refresh_token，POST 到 sso `token_endpoint`（带 client_secret）；不接受请求体传递 refresh token；`invalid_grant`/`invalid_token` → 抛 session 过期（前端跳登录）。
...
- 权威源 = sso（用户/租户/会话/角色）。
- vibecoding/scaffold 本地 `UserInfo` 仅作**缓存映射**：`ssoSub`（`@unique`）作为与 sso 的唯一关联；`nickname/avatar/locale/isAdmin` 等可冗余缓存，但以 sso 为准（`user-sync` 在每次校验/登录时回填）。
- 不在 vibecoding/scaffold 侧实现注册/改密/邮箱验证等——统一走 sso。
- 不在 `@repo/contracts` 暴露本地 `/sign` contract；前端不创建 `signClient`，所有登录、刷新、登出均通过 OIDC client/token-manager 代理到 sso。

### 4.2 文件管理

- 权威源 = sso 文件服务。
- vibecoding/scaffold 作为**消费方**，通过 `@dofe/file-sdk` / `@dofe/file-sdk-web` 调用 sso：上传、CDN URL、元数据。
- **不**复制 sso 的 `FileDomainModule`/`file-api`/`cdn-proxy`/`internal-file-api`（避免 tenant/team 多租户耦合与重复维护）。
- **不**保留本地 `UploaderModule`、`FileSource` 或文件 bucket/env 枚举；`avatarFileId` 仅保存 SSO file id。
- 前端上传统一走 `FileUploader({ apiBase: '/api/proxy/sso' })`，Next rewrite 转发到 `${NEXT_PUBLIC_SSO_BASE_URL}/:path*`；SDK 会自行请求 `/api/uploader/*`。
